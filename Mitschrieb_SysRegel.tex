...
% Tabellarische Auflistung der wichtigen Daten des nachfolgenden Werkes. Nach belieben anpassen.
         ...
         \\
						\begin{tabular}{ll}
                \large \LaTeX ~von William Glover \\
                \large Stand: \today\\
...
\end{equation*}
$G(s)$ heißt \emph{Übertragungsfunktion}. Sie beschreibt dsa System vollständig. Das Übertragungs\-verhalten bei verschwindendem Anfangswert, also $y(s)=G(s)\cdot u(s)$ entspricht $y(t)=g(t)*u(t)$ im Zeitbereich. Es gilt also $G(s)=\mathcal{L}\{g(t)\}$. (Regel 9)\\
...
Bisher blieb die Forderung (\RM{2}) nach stationärer Genauigkeit unberücksichtigt. Es wird nun ein Vorfilter $\mathbf{S}$ eingeführt, der auch diese Forderung mit in die Gleichung einfließen lässt.
\[
u(t)=-\vec{k}\vec{x}(t)+\mathbf{S}\underbrace{\vec{\omega}(t)}_{\text{Führungsvektor}}
\]
Es soll \textbf{S} für $\mathbf{D}=0$ bestimmt werden.
\[
\dot{\vec{x}}=\mathbf{A}\vec{x}+\mathbf{B}\vec{u}=\mathbf{A}\vec{x}+\mathbf{B}(-\vec{k}\vec{x}+\mathbf{S}\vec{\omega})=(\mathbf{A}-\mathbf{B}\vec{k})x+\mathbf{BS}\vec{\omega}
\]
im stationären Zustand ist $\dot{x}=0$
\[
(A-Bk)x=-\mathbf{BS}\omega
\]

\[
\underbrace{\mathbf{C}\vec{x}}_{\text{Ausgangsgl.}=y(t)}=-\mathbf{C}(\mathbf{A}-\mathbf{B}\vec{k})^{-1}\mathbf{BS}\omega
\]
\[
-[\mathbf{C}(\mathbf{A-B}\vec{k})^{-1}\mathbf{B}]^{-1}\vec{y}=\mathbf{S}\vec{\omega}
\]
Nach Forderung (\RM{2}) muss gelten $\vec{y}=\vec{\omega}$
\[
\Rightarrow-[\mathbf{C}(\mathbf{A-B}\vec{k})^{-1}\mathbf{B}]^{-1}=\mathbf{S}
\]

\section{Beobachtung nicht direkt messbarer Systemzustände}

\subsection{Idee und Struktur}
\begin{description}
\item[Problem: ] Für die Zustandsrückführung $\vec{v}(t)=-h\vec{x}(t)$ müssen alle Einträge in $\vec{x}(t)$ messbar sein, was i.A. nicht möglich sein wird. Wie kann man aus den messtechnisch erfassten Größen der gesante Zustandsvektor $\vec{x}(t)$ wenigstens näherungsweise rekonstruieren?
\item[1. Versuch:] Schätzung mit dem inversen Streckenmodell
  \begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{sysregel_No_8}
  \end{figure}
Differenzierung ist sehr Störanfällig! Dieses Modell liefert also nur eine schlechte Schätzung von v.
\item[2. Versuch: ] Schätzung mit Parallelmodell
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{sysregel_No_9}
  \end{figure}
Besser realisierbar, aber  
\begin{itemize}
  \item Anfangswerte der Strecke sind unbekannt.
  \item Beide Systeme laufen ``außeinander'', falls sie nicht exakt übereinstimmen
  \end{itemize}
\item[Versuch 3:] Schätzung mit korrigiertem Streckenmodell

  \begin{figure}[H]
    \centering
    \includegraphics[width=.7\linewidth]{sysregel_No_10}
  \end{figure}
Abweichung der messbaren Größe wird verwendet, um über eine mit H gewichtete Rückführung das parallele Streckenmodell zu korrigieren.
\begin{itemize}
\item Verallgemeinerung: Zustandsraumdarstellung\\
  \begin{table}[H]
    \centering
    \begin{tabular}{lll}
      Strecke: $(\mathbf{D}=\mathbf{0})$&&paralleles Modell\\
      $\dot{\vec{y}}=\mathbf{A}\vec{x}+\mathbf{B}\vec{u}$&&$\dot{\vec{\hat y}}=\mathbf{A}\vec{\hat x}+\mathbf{B}\vec{u}+\mathbf{H}(\vec{y}-\vec{\hat y})$\\
      $\vec{y}=\mathbf{C}\vec{x}$&&$\vec{\hat y}=\mathbf{C}\vec{\hat x}$
    \end{tabular}
  \end{table}
$\vec{\hat x}$ ist dabei eine Schätzung von $\vec{x}$
\item Diese Schätzeinrichtung wird Beobachter genannt ($\rightarrow$ Blatt 6-1)
\end{itemize}
\end{description}

\subsection{Beobachterentwurf}

Wie ist die Beobachtermatrix $\mathbf{H}$ zu wählen?
\begin{description}
\item[Ziel:] $\mathbf{H}$ so wählen, dass der Schätzfehler 
\[
\Delta \vec{x}=\vec{x}-\hat x
\]
für $t\mapsto\infty$ gegen 0 geht.
\end{description}
Die DGL für den Schätzfehler lautet
\[
\Delta\dot{\vec{x}}=\dot{\vec{x}}=\dot{\vec{\hat x}}=\mathbf{A}\vec{x}+\mathbf{B}\vec{u}-\mathbf{A}\vec{x}-\mathbf{B}\vec{u}-\mathbf{H}(\vec{y}-\vec{\hat y})
\]
mit $\vec{y}=\mathbf{C}\vec{x}$, $\vec{\hat y}=\mathbf{C}\vec{\hat x}$
\begin{itemize}
\item Damit der Beobachter stabil ist und $\Delta \vec{x}\mapsto 0$ für $t \mapsto\infty$ geht, müssen die Eigenwerte von $\mathbf{A}_B$ in der linken komplexen Halbebene liegen.
\item Eigenwertvorgabe (ähnlich wie beim Reglerentwurf):
  \begin{enumerate}
  \item Eigenwerte sollen links von den Eigenwerten der geregelten Strecke liegen.
  \item Sind die Eigenwerte zu weit links, reagiert der Beobachter ``nervös" auf störungen.
  \end{enumerate}
\item Regler und Beobachterentwurf sind komplementär! Vgl. Blatt 6-2
\item Vorraussetzung: Beobachtbarkeit der Strecke 
\end{itemize}

\paragraph{Anwendungen des Beobachters}

\begin{itemize}
\item Im Regelkreis
  \begin{itemize}
  \item Schätzung für $\vec{x}$ für die Zustandsrückführung $\rightarrow$ Blatt 6-2
  \item Eigenwerte des Beobachters treten zu denen des geregelten Systems hinzu. 
  \end{itemize}
\item Außerhalb der Regelungstechnik
  \begin{itemize}
  \item Schätzung eines internen, nicht direkt messbaren Zustandes auf Basis des Modells und des (gestörten) Messvektors.
  \item \textbf{Trick:} Der gesuchte Zustand muss in den Zustandsvektor aufgenommen werden
  \end{itemize}
\end{itemize}

\begin{bsp}
  Beschleunigungssensor (aus Übung 1)
\end{bsp}

\begin{figure}[H]
  \centering
 \includegraphics[width=\linewidth]{sysregel_No_12}
\end{figure}
Bewegungsgleichung:
\[
F=m\cdot a =m\cdot \ddot{x}\Rightarrow y=\int{\int{F(\tau)d\tau^2}}
\]
\[
F_C=c\cdot y~~F_d=d\cdot \dot{y}
\]
Es werden ein paar Zahlenwerte eingesetzt: $m=0.22 \mu g~;~d=6,8\cdot 10^{-3}mN\frac{\mu s}{\mu m}~;~c=5.4\cdot 10^{-3}\frac{mN}{\mu m}~;~K_{el}=7.69\frac{V}{N}$\\
Bisheriger Ansatz: $U_0$ ist ein Maß für die Beschleunigung. Stationär gilt:
\[
F=F_C
\]
\[
a\cdot m=c\cdot y=c\cdot\frac{U_0}{K_{el}}
\]
\[
\Rightarrow a=\frac{c}{k_ {el}\cdot m}\cdot U_0
\]
Verfahren um die Beschleunigung mit einem  Beobachter zu bestimmen:
\begin{enumerate}
\item Zustände $X_n$ und deren Ableitungen an den Ein- und Ausgängen der Integratoren einführen. Im Beispiel also $x_1, \dot{x_1}, x_2, \dot{x_2}$
\item Beschleunigung ist Zustand $x_3$. Man erweitert also die Zustandsgleichung um Größen, die zwar zur Beschreibung des Systems nicht nötig sind, aber zur Auswertung interessant sind.
...
\item Mit Simulink simuliert pendelt sich der Beobachter sehr schnell auf den gesuchten Wert ein, während der Sensor sehr stark oszilliert.
\end{enumerate}
\end{document}
